...
import { findBundle, getNetworkId } from "../functions/network.func.js";

import { sendData, sendAirtime } from "../functions/buy.func.js";
...
export async function buyAirtime(req, res) {
 try {
  ...
  if (user.balance < amount || user.balance == amount) {
   console.log(
    `Insufficient balance : You need more than  ₦${amount - user.balance} to make this purchase`
   );
...
export async function buyData(req, res) {
 try {
  let { network, phone, planId } = req.body;
  //find the user
  let user = req.user;
  //respond on error
  if (!user)
  ...
  if (user.balance < bundle.price) {
   console.log(
    `Insufficient balance : You need more than  ₦${bundle.price - user.balance} to make this purchase`
   );
  ...
  const transactionRes = await sendData(
   transId,
   bundle.networkId,
   bundle.id,
   phone
  ...
  //verify if data was successful sent
  const verifyTransactionMeassage = await verifyTransaction(transactionRes, user, trans);
  //record as successful

  //send message to user
  res.status(201).json({
   message: verifyTransactionMeassage,
